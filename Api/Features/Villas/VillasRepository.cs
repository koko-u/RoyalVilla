...
[AutoRegisterService]
public sealed class VillasRepository(NpgsqlDataSource dataSource, ILogger<VillasRepository> logger)
    : IRepository
{
    /// <summary>
    /// Get all villas Information with Pagination
    /// </summary>
    /// <returns>Villa information</returns>
    public async Task<IEnumerable<VillaRow>> GetVillasWithPagingAsync(
        PageQuery pageQuery,
        CancellationToken cancellationToken = default
    )
    {
        var sql = await File.ReadAllTextAsync(
            "Sql/Villas/select_with_paging.sql",
            cancellationToken
        );
        ...
        var cmd = new CommandDefinition(
            commandText: sql,
            parameters: new { pageQuery.Limit, pageQuery.Offset },
    ...
    /// <returns></returns>
    public async Task<VillaRow?> GetVillaByIdAsync(
        int id,
        CancellationToken cancellationToken = default
    )
    {
    ...
    /// <param name="cancellationToken"></param>
    public async Task<VillaRow> CreateVillaAsync(
    ...
    /// <param name="cancellationToken"></param>
    /// <returns></returns>
    public async Task<VillaRow?> UpdateVillaAsync(
        int id,
        CreateOrUpdateVillaDto dto,
        CancellationToken cancellationToken = default
    )
    ...
    /// <param name="cancellationToken"></param>
    /// <returns></returns>
    public async Task<VillaRow?> DeleteVillaAsync(int id, CancellationToken cancellationToken)
    {
